...
If a sequence for which a tagged array is already in _V_ is encountered, a shared item reference _i_ is added to the rump instead, splicing the content of the array within 1115 tag into the existing array (see {{-cbor-packed, Section 5.1}}) and that sequence as well as its non-empty suffixes, ordered by their appearance within the rump, is not added again to _V_.
...
The splice integration tag 1115 is the only integration tag in use "application/dns+cbor" for "packed=0" and "packed=1".

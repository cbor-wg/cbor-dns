...
Any "application/dns+cbor" encoder and decoder MUST support tag TBD28259.
...
Future setup tags for implicit table setups MAY allow for inner tags.
...
Due to the order of strings being important, special care should be taken into account for the order of maps (major type 5, {{-cbor}}) within tag TBD28259.
...
Otherwise, additional sending of message might incur and media type negotiation might fail unnecessarily.
...
[
  ["www", "example", "org"],
  [
    [3600, "www", "example", "org", 5, "svc", "www", "example", "org"],
    [3600, "svc", "www", "example", "org", ip'2001:db8::1']
  ],
  [
    [3600, "example", "org", 2, "org", "example", "org"]
  ],
  []
]
~~~
...
Also note that, e.g., the sequence "www", "example", "org" (appears as index 0) is not referenced as `simple(0)` within index 3 as this is how the _V_ would be read given {{fig:name-compression-example-unpacked}}.
...
~~~ cddl
Packed-Text-String-Suffix-Sequenec-Packed-CBOR = #6.113(
  [
    [*shared-and-argument-item],
...
`packed-dns-message` provided in {{fig:packed-dns-msg}} MUST be interpreted as an invalid message.
...
Note, that the encoder needs to rewrite the references from {{fig:name-compression-example-unpacked}} as the shared argument table `["org", 600]` is prepended to the inner implicit table.
